...
export default function GrammarExam({
  ...
  if (phase === "intro") {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "color-mix(in srgb, var(--color-bg) 92%, transparent)", backdropFilter: "blur(12px)" }}>
        <div className="w-full max-w-[420px] rounded-2xl p-6 flex flex-col items-center gap-5" style={{ border: "1px solid var(--color-border)", background: "var(--color-bg)", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
          ...
          <div className="flex gap-3 w-full">
            <button onClick={onClose} className="flex-1 py-3 rounded-xl font-semibold text-[14px]" style={{ background: "var(--color-primary-soft)", border: "1px solid var(--color-border)", color: "var(--color-text-secondary)" }}>Cancel</button>
            <button onClick={handleStart} className="flex-1 py-3 rounded-xl font-semibold text-[14px] text-white" style={{ background: "linear-gradient(135deg, var(--color-primary), var(--color-accent))" }}>Start Exam</button>
          </div>
        </div>
      </div>
    );
  ...
  if (phase === "result") {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "color-mix(in srgb, var(--color-bg) 92%, transparent)", backdropFilter: "blur(12px)" }}>
        <div className="w-full max-w-[420px] rounded-2xl p-6 flex flex-col items-center gap-5" style={{ border: "1px solid var(--color-border)", background: "var(--color-bg)", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
          <div className="w-20 h-20 rounded-full flex items-center justify-center text-4xl" style={{ background: passed ? "linear-gradient(135deg, var(--color-success), var(--color-accent))" : "linear-gradient(135deg, #EF4444, #DC2626)", boxShadow: passed ? "0 0 32px rgba(46,211,198,0.3)" : "0 0 32px rgba(239,68,68,0.3)" }}>
            {passed ? "🏆" : "💪"}
          </div>
          <div className="text-center">
            <p className="text-[22px] font-sora font-bold" style={{ color: "var(--color-text)" }}>{passed ? "Exam Passed!" : "Not Quite Yet"}</p>
            <p className="text-[13px] mt-1" style={{ color: "var(--color-text-secondary)" }}>{passed ? "Great work!" : `Need ${passingScore}% to pass.`}</p>
          </div>
          <div className="px-6 py-3 rounded-full" style={{ border: passed ? "1px solid rgba(46,211,198,0.25)" : "1px solid rgba(239,68,68,0.25)", background: passed ? "rgba(46,211,198,0.08)" : "rgba(239,68,68,0.08)" }}>
            <span className="text-[24px] font-sora font-bold" style={{ color: passed ? "var(--color-success)" : "#EF4444" }}>{finalScore}%</span>
            <span className="text-[13px] ml-2" style={{ color: "var(--color-text-secondary)" }}>({correctCount}/{questions.length})</span>
          </div>
          {timeLeft === 0 && <p className="text-[12px]" style={{ color: "#F59E0B" }}>⏰ Time ran out!</p>}
          <button onClick={onClose} className="w-full py-3 rounded-xl font-semibold text-[14px] text-white" style={{ background: passed ? "linear-gradient(135deg, var(--color-primary), var(--color-accent))" : "linear-gradient(135deg, #F59E0B, #D97706)" }}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: "var(--color-bg)" }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 relative z-10" style={{ borderBottom: "1px solid var(--color-border)" }}>
        <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center text-[16px]" style={{ background: "var(--color-primary-soft)", border: "1px solid var(--color-border)", color: "var(--color-text-secondary)" }}>&times;</button>
        ...
        <div className={cn("flex items-center gap-1 px-3 py-1 rounded-lg text-[13px] font-bold border", timeWarning ? "bg-red-500/10 border-red-500/25 text-red-400" : "border-transparent")} style={!timeWarning ? { background: "var(--color-primary-soft)", color: "var(--color-text-secondary)" } : {}}>
          ⏰ {timeStr}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 pb-24 max-w-[500px] mx-auto w-full relative z-10">
        <DragDropProvider onDragEnd={handleDragEnd} renderOverlay={renderOverlay}>
          <div className="flex items-center gap-2 mb-4">
            <span className="text-[11px] font-bold" style={{ color: "var(--color-text-secondary)" }}>Question {index + 1} of {questions.length}</span>
          </div>
          ...
          <div className="rounded-2xl p-5 mb-5" style={GRAMMAR_CARD_STYLE}>
            {hasBlank ? (
                ...
                {sentenceParts.map((part, i) => (
                  <React.Fragment key={i}>
                    ...
                    {i < blankCount && (
                      isMultiBlank ? (
                        showingFeedback ? (
                          <span className={cn("inline-block px-2 py-0.5 rounded-lg font-bold text-[14px] border",
                          ...
                          <button onClick={() => handleClearSlot(i)} className="inline-flex px-2 py-0.5 rounded-lg font-bold text-[13px] border cursor-pointer hover:opacity-80" style={{ borderColor: "rgba(46,211,198,0.4)", background: "rgba(46,211,198,0.1)", color: "var(--color-success)" }}>
                            {blankAnswers[i]} ×
                          </button>
                        ) : (
              ...
              <p className="text-[15px] font-semibold leading-relaxed" style={{ color: "var(--color-text)" }}>{current.sentence}</p>
            )}
          </div>
          ...
          {isMultiBlank && phase === "question" && isAnswered && (
            <button onClick={handleSubmitMultiBlank} className="w-full py-3 rounded-xl font-semibold text-[14px] text-white mb-5" style={{ background: "linear-gradient(135deg, var(--color-primary), var(--color-accent))" }}>
              Check Answer
            </button>
          )}
          ...
          {showingFeedback && !isCorrect && isMultiBlank && (
            <div className="rounded-xl p-3 mb-4" style={{ border: "1px solid rgba(16,185,129,0.2)", background: "rgba(16,185,129,0.05)" }}>
              <p className="text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color: "#10B981" }}>Correct Answer</p>
              <p className="text-[13px] font-semibold" style={{ color: "var(--color-text)" }}>{correctParts.join(" + ")}</p>
            </div>
          )}
          ...
          {showingFeedback && (
            <button onClick={handleNext} className="w-full py-3.5 rounded-xl font-semibold text-[14px] text-white transition-all hover:opacity-90" style={{ background: "linear-gradient(135deg, var(--color-primary), var(--color-accent))" }}>
